# Generated by Powerlevel10k configuration wizard on 2025-03-24 at 11:53 CET.
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    kotlin                  # kotlin logo (https://kotlinlang.org/)
    ...
    # time                  # current time
    asm                     # assembly language indicator
    # =========================[ Line #2 ]=========================
    newline                 # \n
  ...
  # Vérifie si la variable d'environnement NIX_ENV_NAME existe
  if [[ -n "$NIX_ENV_NAME" ]]; then
    # Si la variable existe, affiche le nom de l'environnement
    typeset -g POWERLEVEL9K_NIX_SHELL_CONTENT_EXPANSION="$NIX_ENV_NAME"
  else
    # Si la variable n'existe pas, affiche uniquement le logo Nix
  ...
  # Custom content.
  typeset -g POWERLEVEL9K_GO_VERSION_CONTENT_EXPANSION=''
  ...
  #
  typeset -g POWERLEVEL9K_JAVA_VERSION_CONTENT_EXPANSION=''
  # Custom icon.
  ...
  # Custom Kotlin segment
  function prompt_kotlin() {
    # Check for Kotlin files in the current directory
    if [[ -n *.kt(#qN) || -n *.kts(#qN) || -f build.gradle.kts ]] ; then
      p10k segment -f 99 -i '🅺' -t ''
    fi
  }

  # Custom Assembly segment
  function prompt_asm() {
    # Check for Assembly files in the current directory
    if [[ -n *.asm(#qN) || -n *.s(#qN) ]] ; then
      p10k segment -f 99 -i '🔨' -t ''
    fi
  }
